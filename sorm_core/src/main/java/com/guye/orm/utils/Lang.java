...
public abstract class Lang {
    ...
    public static <F, T> T cast( Object src, Class<F> fromType, Class<T> toType )
        ...
        if (toType.isAssignableFrom(Collection.class)) {
            if (fromType.isArray()) {
                ...
                try {
                    ...
                    throw new DaoException("can not cast to:" + toType.getName());
                }
            }
            if (src instanceof Collection) {
                try {
        ...
        throw new DaoException(fromType==null?"":fromType.getName() + "can not cast to:" + toType.getName());
    }
    ...
    public static <T> void each( Object obj, Each<T> callback ) {
        ...
        if (obj.getClass().isArray()) {
            ...
            for (int i = 0; i < len; i++) {
                if (callback.invoke(i, (T) Array.get(obj, i), len) == Each.EXIT) {
                    break;
                }
            }
            ...
            for (Iterator<T> it = ((Collection) obj).iterator(); it.hasNext();)
                if (callback.invoke(i++, it.next(), len) == Each.EXIT) {
                    break;
                }
            ...
            int i = 0;
            while (it.hasNext()) {
                if (callback.invoke(i++, (T) it.next(), -1) == Each.EXIT) {
                    break;
                }
            }
        } else
